...
function extractMaterialId(alert) {
  const directId =
    alert?.material_id ||
    alert?.materialId ||
    alert?.id ||
...
function ProductsNotificationCenter() {
  ...
  React.useEffect(() => {
    ...
    async function loadAlerts() {
      ...
      try {
        const data = await apiRequest("/api/all-alerts", { signal: controller.signal })
        if (!active) return
  ...
  function selectAlert(alert) {
    ...
    const materialId = extractMaterialId(alert)
    if (materialId) {
  ...
  return (
    <div className="products-notifications" ref={wrapperRef}>
      <Button
        type="button"
        variant="outline"
        size="icon"
        className="products-button products-notifications-trigger"
      ...
      >
        ...
        {unseenCount > 0 && (
          <span className="products-notifications-count" aria-label={`${unseenCount} new alerts`}>
      ...
      <AnimatePresence>
        {open && (
          <>
            <motion.button
              type="button"
              className="products-notifications-backdrop"
            ...
            <motion.section
              ref={panelRef}
              className="products-notifications-panel"
              role="dialog"
            ...
            >
              <div className="products-notifications-heading">
                <div>
                  <span className="products-eyebrow">Notification center</span>
                  <h2>System alerts</h2>
                </div>
                <Badge variant="secondary">{totalAlerts} active</Badge>
              </div>

              <div className="products-notifications-body" aria-live="polite">
                {loading && <p className="products-notifications-state">Loading alerts...</p>}
                {!loading && error && (
                  <p className="products-notifications-state products-notifications-state--error">
                    {error}
                  </p>
                )}
                {!loading && !error && totalAlerts === 0 && (
                  <p className="products-notifications-state">No current alerts.</p>
                )}
                ...
                {!loading &&
                  ...
                  Object.entries(groups).map(([category, alerts]) => {
                    ...
                    return (
                      <section className="products-alert-group" key={category}>
                        <div className="products-alert-group-heading">
                        ...
                        {items.length === 0 && (
                          <p className="products-alert-empty">No current alerts</p>
                        )}
                        {items.slice(0, isExpanded ? items.length : 1).map((alert, index) => (
                          <button
                            type="button"
                            className="products-alert-item"
                          ...
                          >
                            ...
                            {extractMaterialId(alert) && <ArrowUpRight aria-hidden="true" />}
                          </button>
                        ))}
                        {items.length > 1 && (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className="products-button products-alert-toggle"
...
export default ProductsNotificationCenter
